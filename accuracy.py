...
def run_trial(model, real_amp, real_damp, real_angf, steps, timespan, thin_percentage, cutoff, num_iterations, noise_amplitude):
    ...
    samples = model.metropolis_hastings(data=real_wave, num_iterations=num_iterations, noise_amplitude=noise_amplitude)
...
for noise in NOISE_AMPLITUDE:
    rms = run_trial(model, REAL_AMPLITUDE, REAL_DAMPING, REAL_ANGULAR_FREQ, DATA_STEPS, TIMESPAN, THIN_PERCENTAGE, CUTOFF_START, NUM_ITERATIONS, noise)
